        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-md-10 offset-1">
              <h3 class="text-secondary">導遊工作申請管理</h3>
            </div>
          </div>
          <hr>
          <div class="row">
            <div class="col-md-10 offset-1">
              ...
              <div class="tab-content" id="nav-tabContent">
                <!-- The tourist guide job application status is "Submitted" -->
                <div class="tab-pane fade show active" id="nav-submitted" role="tabpanel" aria-labelledby="nav-submitted-tab">
                  <table id="submitted" class="table table-hover">
                    ...
                    <tbody>
                      <?php
                      $sql = "SELECT `id`, CONCAT(`chinese_lastname`, ' ', `chinese_firstname`) AS `name`, `submit_datetime`
                                FROM `tourist_guide_application`
                                WHERE `status` = 1;";
                      $rs = mysqli_query($conn, $sql);

                      while ($rc = mysqli_fetch_assoc($rs)) {
                        echo '<tr onclick="wt_under_review(' . $rc['id'] . ')"><td>' . $rc['id'] . "</td>";
                        echo "<td>" . $rc['name'] . "</td>";
