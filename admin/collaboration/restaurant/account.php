...
<head>
  ...
  <title>餐廳合作夥伴帳號 (ID:<?php echo $account_id; ?>) - 管理頁面</title>
...
<body>
                  ...
                  <div class="row">
                    <div class="col-md-8">
                      <h3 class="card-title text-secondary">
                        餐廳合作夥伴帳號資料 <?php echo '(ID:'.$rc['account_id'].')'; ?>
                      </h3>
                    </div>
            ...
            <div class="col-md-10 offset-1">
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <h3 class="card-title text-secondary">持有的餐廳列表</h3>
                    </div>
                  </div>
                  <!-- Table -->
                  <div class="row">
                    <div class="col-md-12">
                      <table id="restaurantList" class="table table-hover">
                        <thead>
                          <th>ID</th>
                          <th>中文名稱</th>
                          <th>英文名稱</th>
                        ...
                        <tbody>
                          ...
                          $sql = "SELECT
                                    `restaurant`.`restaurant_id` AS `id`,
                                    `restaurant`.`restaurant_chinese_name` AS `chi_name`,
                                    `restaurant`.`restaurant_english_name` AS `eng_name`,
                                    `district`.`zh-hk` AS `district`,
                                    `restaurant`.`create_datetime` AS `create_date`,
                                    `status`.`zh-hk` AS `status`
                                  FROM `restaurant` `restaurant`
                                  INNER JOIN `hong_kong_district` `district` ON `restaurant`.`district` = `district`.`district_id`
                                  INNER JOIN `restaurant_status` `status` ON `restaurant`.`status` = `status`.`status_id`
                                  WHERE `restaurant`.`partner_id` = {$rc['account_id']}
                                  ORDER BY `create_date` ASC;";
                          $rs = mysqli_query($conn, $sql);
                          while ($restaurant = mysqli_fetch_assoc($rs)) {
                            echo '<tr onclick="window.location.assign(\'/admin/site/restaurant/restaurant_editor.php?id='.$restaurant['id'].'\')">';
                            echo "<td>".$restaurant['id']."</td>";
                            echo "<td>".$restaurant['chi_name']."</td>";
                            echo "<td>".$restaurant['eng_name']."</td>";
                            echo "<td>".$restaurant['district']."</td>";
                            echo "<td>".$restaurant['create_date']."</td>";
                            echo "<td>".$restaurant['status']."</td>";
                            echo "</tr>";
                          }
                          ?>
                        </tbody>
  ...
  <script>
    ...
    let year_selected = <?php if ($rc['birth_year'] == '') { echo 1901; } else { echo $rc['birth_year']; } ?>;
