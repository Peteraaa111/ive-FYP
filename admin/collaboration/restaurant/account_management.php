        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-md-10 offset-1">
              <h3 class="text-secondary">餐廳合作夥伴帳號資料概覽</h3>
            </div>
          </div>
          <hr>
          <div class="row">
              ...
              <table id="myTable" class="table table-hover">
                ...
                <tbody>
                  ...
                  $sql = "SELECT
                            ...
                            r1.`restaurant_chinese_name` AS `first_restaurant`,
                            r2.`count` AS `total_restaurant`,
                            s.`zh-hk` AS `status`,
                            ac.`registration_time` AS `registration_time`
                          FROM `account` ac
                          INNER JOIN `account_status` s ON ac.`status` = s.`status_id`
                          INNER JOIN `restaurant` r1 ON ac.account_id = r1.partner_id
                          INNER JOIN (
                              SELECT
                                `partner_id`,
                              ...
                              GROUP BY `partner_id`
                          ) r2 ON ac.`account_id` = r2.`partner_id` AND r1.`create_datetime` = r2.`minDate`
                          WHERE ac.`type_id` = 2
                  ...
                  while ($rc = mysqli_fetch_assoc($rs)) {
                    ...
                    echo "<td>" . $rc['first_restaurant'] . "</td>";
                    echo "<td>" . $rc['total_restaurant'] . "</td>";
