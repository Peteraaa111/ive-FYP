        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-md-10 offset-1">
              <h3 class="text-secondary">民宿合作夥伴帳號資料概覽</h3>
            </div>
          </div>
          <hr>
          <div class="row">
              ...
              <table id="myTable" class="table table-hover">
                <thead>
                  <tr>
                    ...
                    <th scope="col">首間民宿名稱</th>
                    <th scope="col">民宿數量</th>
                ...
                <tbody>
                  ...
                  $sql = "SELECT
                            ...
                            CONCAT(ac.`lastname`, ' ', ac.`firstname`) AS `name`,
                            IF(ac.`gender` = 'm', '男', '女') AS `gender`,
                            ac.`phone_number` AS `phone_number`,
                            g1.`guesthouse_chinese_name` AS `first_guesthouse`,
                            g2.`count` AS `total_guesthouse`,
                            s.`zh-hk` AS `status`,
                            ac.`registration_time` AS `registration_time`
                          FROM `account` ac
                          INNER JOIN `account_status` s ON ac.`status` = s.`status_id`
                          INNER JOIN `guesthouse` g1 ON ac.account_id = g1.partner_id
                          INNER JOIN (
                              SELECT
                                `partner_id`,
                                `guesthouse_chinese_name`,
                                MIN(`create_datetime`) AS minDate,
                                COUNT(`partner_id`) AS `count`
                              FROM `guesthouse`
                              WHERE partner_id IS NOT NULL
                              GROUP BY `partner_id`
                          ) g2 ON ac.`account_id` = g2.`partner_id` AND g1.`create_datetime` = g2.`minDate`
                          WHERE ac.`type_id` = 3
                  ...
                  while ($rc = mysqli_fetch_assoc($rs)) {
                    ...
                    echo "<td>" . $rc['first_guesthouse'] . "</td>";
                    echo "<td>" . $rc['total_guesthouse'] . "</td>";
                ...
                <tfoot>
                  <tr>
                    ...
                    <th scope="col">性別</th>
                    <th scope="col">聯絡電話</th>
                    <th scope="col">首間餐廳名稱</th>
                    <th scope="col">餐廳數量</th>
                    <th scope="col">帳號狀態</th>
                    <th scope="col">註冊時間</th>
                  </tr>
                </tfoot>
