    ...
    <div class="row">
          ...
          <div class="card-body">
            ...
            <div id="selected-label" class="hidden">
              <ul class="selected-badge">
                ...
                <?php
                if (isset($_GET['what']) || isset($_GET['district']) || isset($_GET['type'])) {
                ...
                if (isset($_GET['type'])) {
                  ...
                  for ($i=0; $i<$length; $i++) {
                    $sql = "SELECT * FROM `attraction_type_list` WHERE `type_id` = ".$_GET['type'][$i];
                    $rs = mysqli_query($conn, $sql);
                    $rc = mysqli_fetch_assoc($rs);
                    echo '
                    <span id="'.$rc['type_id'].'" class="badge bg-success type-label" data="'.$rc['type_id'].'">'.$rc['zh-hk'].'
                    <button type="button" class="btn-close btn-close-white" aria-label="Close"></button>
                    </span>
                    ';
                  }
            ...
            <section id="input-selector">
              <h6 class="text-secondary">搜尋景點名稱</h6>
            ...
            <section id="type-selector">
              ...
              <ul>
                <?php
                $typesql = "SELECT att.`type_id`, COUNT(att.`type_id`) as `total`, atl.`zh-hk` AS `chi-name`, atl.`en` AS `en-name`
                              FROM `attraction_type` att
                              INNER JOIN `attraction_type_list` atl ON att.`type_id` = atl.`type_id`
                              INNER JOIN `attraction` a ON att.`attraction_id` = a.`attraction_id`
                              WHERE a.`status` = 1
                              GROUP BY `type_id`
                              ORDER BY `total` DESC LIMIT 5;";
      ...
      <div class="col-md-6">
          <ul id="result" onload="search();">
            ...
            $start_from = ($page - 1) * 6;
            // Default sql
            $sql = "SELECT DISTINCT a.`attraction_id` AS `id`, a.`attraction_chinese_name` AS `chi-name`, a.`chinese_address` AS `chi-address`, atl.`zh-hk` AS `typeName`
                        FROM `attraction` a
                        INNER JOIN `attraction_type` att ON a.`attraction_id` = att.`attraction_id`
                        INNER JOIN `attraction_type_list` atl ON att.`type_id` = atl.`type_id`
                        WHERE `status` = 1
                        GROUP BY `id`
            ...
            if ($hasWhat || $hasDistrict || $hasType) {
              
              // Initial the sql for WHERE clause
              $sql = "SELECT
                        a.`attraction_id` AS `id`,
                        a.`attraction_chinese_name` AS `chi-name`,
                        a.`chinese_address` AS `chi-address`,
                        hkd.`zh-hk` AS `district`,
                        atl.`zh-hk` AS `typeName`
                      FROM `attraction` a
                      INNER JOIN `hong_kong_district` hkd ON a.`district` = hkd.`district_id`
                      INNER JOIN `attraction_type` att ON a.`attraction_id` = att.`attraction_id`
                      INNER JOIN `attraction_type_list` atl ON att.`type_id` = atl.`type_id`
                      WHERE `status` = 1 AND ";
              if ($hasWhat) {
                $sql .= "a.attraction_chinese_name LIKE '%".$_GET['what']."%'";
                if ($hasDistrict || $hasType) {
                  $sql .= " AND ";
                }
              }
              if ($hasDistrict) {
                $length = count($_GET['district']);
                for ($i=0; $i<$length; $i++) {
                  $sql .= "a.`district` = '".$_GET['district'][$i]."'";
            ...
            while ($rc = mysqli_fetch_assoc($rs)) {
              ...
              if (isset($_SESSION['user_id'])) {
                $onclick = 'log('.$_SESSION['user_id'].', '.$rc['id'].', \'CLICK\', \'\', \'attraction\')';
              }
              echo '
              <li style="list-style-type: none";>
                <a href="/'.$_COOKIE['lang'].'/a_details.php?id='.$rc['id'].'" onclick="'.$onclick.'">
                  <div class="card" style="margin-bottom: 10px;">

                        <div class="row g-0">
                          <div class="col-md-4" style="height: 160px;">
                            <img src="/data/site/attraction/'.$rc['id'].'/banner.jpg" class="img-fluid rounded-start" style="height: 100%;"></img>
                          </div>
