$(document).ready(function () {
  ...
  // Step 2 Button - Next to restaurant button
  $('#next-to-restaurant').click(function () {
    ...
    if (!email_emptyChecking(email)) {
      ...
      $.ajax({
        url: "/function.php?op=collabUserRegister&type=2",
        method: "POST",
        dataType: "json",
  ...
  $('#next-to-finish').click(function () {
    $.ajax({
      url: "/function.php?op=siteGenerator&type=restaurant&account=" + new_ac_id,
      method: "POST",
      dataType: "json",
      data: $('#collab-restaurant').serialize(),
      success: function (res) {
        if (res.success === true) {
