...
function cancelItineraryBooking() {
  ...
  var booking_status = document.getElementById("booking-status").value;
  var driver_id = document.getElementById("driver-id").value;
  var tourguidde_id = document.getElementById("tourguide-id").value;
  ...
  if (booking_status == 1 || driver_id != "" || tourguidde_id != "") {
    Toast.fire({
      title: '因訂單正在處理，所以未能取消。'
    });
    ...
    Toast.fire({
      title: '因訂單已被取消，所以無法更新。'
    });
  } else {
    // You need to use standard javascript object here
  var form = $("#update-itinerary-booking-form")[0];
  var formData = new FormData(form);

  $.ajax({
    url: "/itinerary_function.php?op=cancelItineraryBooking",
    method: "POST",
    dataType: "json",
    data: formData,
    processData: false,
    ...
    success: function (res) {
      if (res.success === true) {
        Swal.fire({
          ...
          window.location.replace("/zh-hk/membership/itineray_booking.php");
        });
      } else {
        Swal.fire({
          icon: 'error',
